...
def get_late_approver(employee, page):
    ...
    lateApproverList = frappe.db.get_list(
        ...
        filters=filters,
        fields=['name', 'employee', 'employee_name', 'late_date', 'docstatus', 'workflow_state', 'reason'],
...
def create_late_approver(employee, late_date, reason):
    ...
    return "Late approver request created"
...
def update_late_approver(name, late_date, reason):
    ...
    return "Late Approver updated"

@frappe.whitelist()
def late_approval_up_sbm(name, action):
    doc = frappe.db.get_list("Late Approval Request", filters={"name": name}, fields=["*"])
